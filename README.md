...
Developer-friendly & type-safe Typescript SDK specifically catered to leverage *holiday-with-readme* API.

<div align="left">
    <a href="https://www.speakeasy.com/?utm_source=holiday-with-readme&utm_campaign=typescript"><img src="https://custom-icon-badges.demolab.com/badge/-Built%20By%20Speakeasy-212015?style=for-the-badge&logoColor=FBE331&logo=speakeasy&labelColor=545454" /></a>
...
## Table of Contents
<!-- $toc-max-depth=2 -->
* [holiday-with-readme](#holiday-with-readme)
...
```bash
npm add holiday-with-readme
```

### PNPM

```bash
pnpm add holiday-with-readme
```

### Bun

```bash
bun add holiday-with-readme
```

### Yarn

```bash
yarn add holiday-with-readme zod
...
import {
  HTTPValidationError,
  SDKValidationError,
} from "holiday-with-readme/models/errors";
...
async function run() {
  let result;
  try {
    result = await holidayWithReadme.destinations.getDestinations({});
...
```typescript
import { HolidayWithReadme } from "holiday-with-readme";
import { HTTPClient } from "holiday-with-readme/lib/http";
...
const sdk = new HolidayWithReadme({ httpClient });
```
...
const sdk = new HolidayWithReadme({ debugLogger: console });
```

You can also enable a default debug logger by setting an environment variable `HOLIDAYWITHREADME_DEBUG` to true.
<!-- End Debugging [debug] -->
...
### SDK Created by [Speakeasy](https://www.speakeasy.com/?utm_source=holiday-with-readme&utm_campaign=typescript)
